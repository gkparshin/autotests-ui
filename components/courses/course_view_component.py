...
class CourseViewComponent(BaseComponent):
    ...
    def check_visible(self, index: int, title: str, max_score: str, min_score: str, estimated_time: str):
        ...
        expect(self.max_score_text.nth(index)).to_have_text('Max score: ' + max_score)

        expect(self.min_score_text.nth(index)).to_be_visible()
        expect(self.min_score_text.nth(index)).to_have_text('Min score: ' + min_score)

        expect(self.estimated_time_text.nth(index)).to_be_visible()
        expect(self.estimated_time_text.nth(index)).to_have_text('Estimated time: ' + estimated_time)